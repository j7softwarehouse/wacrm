-- ============================================================
-- 043_notifications_pt_br
--
-- O trigger de notificação de atribuição de conversa (migração 027)
-- grava `title`/`body` em inglês, fixo, direto no INSERT — não é uma
-- chave de tradução que a UI resolve, é texto literal que aparece do
-- jeito que foi gravado. Este deployment roda inteiramente em pt-BR
-- (NEXT_PUBLIC_APP_LOCALE=pt); o texto do banco precisa acompanhar.
--
-- CREATE OR REPLACE FUNCTION é idempotente por natureza — seguro
-- re-executar. Não mexe na tabela, nas políticas RLS nem no trigger em
-- si, só no texto que a função grava.
-- ============================================================
