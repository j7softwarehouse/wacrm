...
// The base URL we publish invite links under is derived by the shared
// `getBaseUrl` helper (`@/lib/http/base-url`), which used to live here
// as a local function. It moved when the UAZAPI channel routes needed
// the same proxy-aware resolution to publish their inbound-webhook
// URLs — see that module for the full resolution order and the
// `ALLOWED_INVITE_HOSTS` rationale.
//
...
// marketing site, which 404s on `/join/<token>`. The resolution
// chain in that module removes the foot-gun.
