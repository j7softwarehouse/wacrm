...
async function processWebhook(body: { entry?: WhatsAppWebhookEntry[] }) {
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      ...
      const channel = configRows[0] as WhatsAppChannel

      const decryptedAccessToken = decrypt(channel.access_token!)
...
/**
 * The Meta-side translation layer.
 *
 * Everything provider-specific about an inbound message stops here:
 * the payload shape, and the media-URL resolution (which verifies the
 * media id against the Graph API before building our proxy URL). What
 * crosses into `ingestInboundMessage` is a provider-neutral
 * `InboundContent` with any media URL already resolved.
 */
async function ingestMetaMessage(
  message: WhatsAppMessage,
  contact: { profile: { name: string }; wa_id: string },
  channel: WhatsAppChannel,
  accessToken: string
) {
  const content = await buildInboundContent(message, accessToken)
  ...
  await ingestInboundMessage(supabaseAdmin(), {
    ...
    content,
    // Present when the customer swipe-replies to one of our messages;
    // the core resolves it to an internal UUID, conversation-scoped.
    replyToProviderMessageId: message.context?.id,
  })
}

// The inbound types the core models directly. Anything Meta sends that
// isn't here (a new/unsupported type) is carried as 'text' — matching
// the pre-extraction fallback, where an unknown type was written to
// `messages.content_type` as 'text' with parseMessageContent's
// "[Unsupported message type: X]" as the body.
const INBOUND_CONTENT_TYPES = new Set<InboundContent['type']>([
  'text',
  'image',
  'video',
  'document',
...
function toInboundType(metaType: string): InboundContent['type'] {
  return INBOUND_CONTENT_TYPES.has(metaType as InboundContent['type'])
    ? (metaType as InboundContent['type'])
    : 'text'
}
...
async function buildInboundContent(
  ...
  // Reactions skip parseMessageContent entirely — there's no media to
  // resolve and no text to store. They're per-(target, actor) state,
  // which the core applies to `message_reactions`. Skipping the parse
  // also skips the media-verification round trip, exactly as before.
  ...
  return {
    type: toInboundType(message.type),
    text: contentText ?? undefined,
    mediaUrl: mediaUrl ?? undefined,
    // Carried for completeness; the messages table has no media_type
    // column, so the core ignores it. The MIME type only matters here,
    // while building the proxy URL.
